...
export class ListWrapper {
  ...
  // keep both methods.
  static createFixedSize(size: number): any[] { return new List(size); }
  static createGrowableSize(size: number): any[] { return new List(size); }
  static get<T>(m: T[], k: number): T { return m[k]; }
  static set<T>(m: T[], k: number, v: T) { m[k] = v; }
  static clone<T>(array: T[]): T[] { return array.slice(0); }
  static map<T, V>(array: T>, fn: (T) => V): List<V[] { return array.map(fn); }
  ...
  static concat(a: any>, b: List<any>): List<any[] { return a.concat(b); }
  ...
  static removeAll<T>(list: T>, items: List<T[]) {
  ...
  static equals(a: any>, b: List<any[]): boolean {
  ...
  static slice<T>(l: T>, from: number = 0, to: number = null): List<T[] {
    return l.slice(from, to === null ? undefined : to);
  }
  static splice<T>(l: T>, from: number, length: number): List<T[] {
...
export class SetWrapper {
  static createFromT>(lst: List<T>): Set<T[] { return createSetFromList(lst); }
