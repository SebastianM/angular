...
function _constructDependencies(factoryFunction: Function, dependencies: any[]):
    Dependency[] {
  if (isBlank(dependencies)) {
    return _dependenciesFor(factoryFunction);
  } else {
    var params: List<any>> = ListWrapper.map(dependencies, (t) =[] [t]);
...
function _extractToken(typeOrFunc, metadata /*any> | any*/, params: List<List<any>[]):
    Dependency {
  var depProps = [];
  var token = null;
  var optional = false;
