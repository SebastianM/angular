...
export class BindingRecordsCreator {
  ...
  getPropertyBindingRecords(textBindings: ASTWithSource[],
                            elementBinders: RenderElementBinder[],
                            allDirectiveMetadatas:
                                RenderDirectiveMetadata>): List<BindingRecord[] {
    var bindings = [];
  ...
  getDirectiveRecords(elementBinders: RenderElementBinder[],
                      allDirectiveMetadatas: RenderDirectiveMetadata>): List<DirectiveRecord[] {
  ...
  _createTextNodeRecords(bindings: BindingRecord>, textBindings: List<ASTWithSource[]) {
...
function _collectNestedProtoViews(
    ...
    result: RenderProtoViewWithIndex> = null): List<RenderProtoViewWithIndex[] {
  if (isBlank(result)) {
    result = [];
  }
...
function _getChangeDetectorDefinitions(
    ...
    nestedPvsWithIndex: RenderProtoViewWithIndex>, nestedPvVariableNames: List<List<string>[],
    allRenderDirectiveMetadata: RenderDirectiveMetadata[], genConfig: ChangeDetectorGenConfig):
    ChangeDetectorDefinition[] {
...
function _createAppProtoView(
    ...
    variableBindings: Map<string, string>, allDirectives: DirectiveBinding[],
...
function _collectNestedProtoViewsVariableBindings(
    nestedPvsWithIndex: RenderProtoViewWithIndex>): List<Map<string, string>[] {
  return ListWrapper.map(nestedPvsWithIndex, (pvWithIndex) => {
...
function _createVariableNames(parentVariableNames: string>, renderProtoView): List<string[] {
  var res =
      isBlank(parentVariableNames) ? <string>[][] : ListWrapper.clone(parentVariableNames);
  MapWrapper.forEach(renderProtoView.variableBindings,
